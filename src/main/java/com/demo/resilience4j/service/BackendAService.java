package com.demo.resilience4j.service;
...
import com.demo.resilience4j.exception.BusinessException;
...
public class BackendAService implements Service {
	...
	public Mono<String> monoTimeout() {
		return Mono.just("Hello World from backend A").delayElement(Duration.ofSeconds(10));
	}
	...
	/**
	 * Spring Boot 스타터는 자동으로 구성된 주석과 AOP Aspect를 제공합니다.
	 * RateLimiter, Retry, CircuitBreaker 및 Bulkhead 주석은 동기 반환 유형과 CompletableFuture와 같은 비동기 유형 및 Spring Reactor의 Flux 및 Mono와 같은 반응 유형을 지원합니다.
	 */
	@Override
	@Bulkhead(name = BACKEND_A, type = Type.THREADPOOL)
	@TimeLimiter(name = BACKEND_A)
